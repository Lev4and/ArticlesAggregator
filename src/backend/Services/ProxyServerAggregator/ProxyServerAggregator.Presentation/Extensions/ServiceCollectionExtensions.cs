...
using ProxyServerAggregator.Presentation.Constants;
using ProxyServerAggregator.Presentation.Endpoints;
using ProxyServerAggregator.Presentation.ExceptionHandlers;
        ...
        services
            .AddSwaggerGen(options =>
            {
                ...
                options.SwaggerDoc(
                    ...
                    new OpenApiInfo
                    {
                        Title       = "ProxyServerAggregator",
                        Description = "Proxy Server Aggregator API",
                        Version     = "1.0.0"
                    });
                ...
                options.IncludeXmlComments(
                    filePath: AppContext.BaseDirectory + "/ProxyServerAggregator.Domain.xml", 
                    includeControllerXmlComments: true);
                
                options.IncludeXmlComments(
                    filePath: AppContext.BaseDirectory + "/ProxyServerAggregator.Application.xml", 
                    includeControllerXmlComments: true);
            
                options.IncludeXmlComments(
                    filePath: AppContext.BaseDirectory + "/ProxyServerAggregator.Presentation.xml", 
                    includeControllerXmlComments: true);
            })
